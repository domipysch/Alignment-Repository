...
and produce a CSV where:
 ...
 - the first column (rows) are the cell names

Usage:
    python createGepMatrixFromSparse.py /path/to/01_HumanBreastCancer_CID4290 \
...
def main(argv=None) -> int:
    ...
    cells_path = os.path.join(dataset_dir, "scData_Cells.csv")
    genes_path = os.path.join(dataset_dir, "stData_Genes.csv")
    mtx_path = os.path.join(dataset_dir, "scData_Sparse.mtx")

    for p in (cells_path, genes_path, mtx_path):
        if not os.path.exists(p):
            print(f"Error: required file missing: {p}", file=sys.stderr)
            return 3

    cells = read_names(cells_path)
    genes = read_names(genes_path)

    print(f"Read {len(cells)} cells and {len(genes)} genes")
    ...
    # We expect final CSV to have rows=cells, columns=genes (data shape: n_cells x n_genes)
    n_cells = len(cells)
    n_genes = len(genes)

    # Determine orientation and transpose if necessary
    if (r, c) == (n_genes, n_cells):
        print("MTX appears to be genes x cells -> transposing to cells x genes")
        if issparse(mat):
            mat = mat.T.tocsr()
        else:
            mat = mat.T
    elif (r, c) == (n_cells, n_genes):
        print("MTX appears to be cells x genes -> keeping orientation")
    else:
        # If shape doesn't match, try to coerce or raise an informative error
        msg = (
            f"MTX shape {mat.shape} does not match lengths: "
            f"genes={n_genes}, cells={n_cells}."
        )
        print("Warning: " + msg, file=sys.stderr)

        # Try transpose and re-check
        if (c, r) == (n_genes, n_cells):
            print("After transposing the MTX it matches gene/cell counts -> transposing")
            if issparse(mat):
                mat = mat.T.tocsr()
            else:
                mat = mat.T
    ...
    # Sanity check shapes
    if arr.shape != (n_cells, n_genes):
        print(f"Error: final data shape {arr.shape} != (cells, genes) ({n_cells},{n_genes})", file=sys.stderr)
        return 5

    # Build DataFrame with index=cells and columns=genes
